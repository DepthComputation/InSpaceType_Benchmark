...
def test(params):
    ...
    with torch.no_grad():
        for _, sample in enumerate(tqdm(dataloader.data)):
            ...
            pred_depth = depth_est.cpu().squeeze()#.numpy()
    ...
    for i in range(1):
        f = open(f'evaluation-newcrfs-all.txt','w')
    ...
    for i in range(4):
        f = open(f'evaluation-newcrfs-H0_{i+1}.txt','w')
    ...
    for i in range(12):
        f = open(f'evaluation-newcrfs-H1_{i+1}.txt','w')
    ...
    for i in range(27):
        f = open(f'evaluation-newcrfs-H2_{i+1}.txt','w')
