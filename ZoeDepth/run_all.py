...
def run(input_path, output_path, model_path, model_type="dpt_hybrid", optimize=True):
    ...
    meta = pd.read_csv('/home/choyingw/Documents/ZED/1000_meta_H.csv')
    ...
    #     local_metr.append(AverageMeter('a2'))
    #     local_metr.append(AverageMeter('a3'))
    #     all_metr.append(local_metr)
    depth_metric_names = [
        "de/abs_mn", "de/abs_rel", "de/sq_rel", "de/rms", "de/log_rms", "da/a1", "da/a2", "da/a3"]
    #metric_set = [abs_mn, abs_rel, sq_rel, rms, log_rms, a1, a2, a3]
    N=1
    ...
    for ind, img_name in enumerate(img_names):
        ...
        # cat = meta['cat'][ind]
        # metric_set = all_metr[cat-1]

        # for i, var in enumerate(metric_set):
        #     var.update(np.array(depth_errors[i].cpu()), 1)

    idfy = model_type

    for i in range(1):
        f = open(f'evaluation-large-{idfy}-global_{i}.txt','w')
    ...
    for i in range(4):
        f = open(f'evaluation-large-{idfy}-H0_{i}.txt','w')
    ...
    for i in range(12):
        f = open(f'evaluation-large-{idfy}-H1_{i}.txt','w')
    ...
    for i in range(27):
        f = open(f'evaluation-large-{idfy}-H2_{i}.txt','w')
    ...
    # for i in range(12):
    #     f = open(f'evaluation-large-{idfy}-cat_{i+1}.txt','w')
    #     metric_set = all_metr[i]
    #     for num in metric_set:
    #         f.write(str(num))
    print("finished")
